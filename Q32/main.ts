...
let currentUsers: string[] = ["john", "paul", "george", "ringo", "mike"];
let newUsers: string[] = ["John", "Paul", "mary", "ringo", "emily"];

for (let new_user of newUsers) {
  if (currentUsers.includes(new_user.toLowerCase())) {
    console.log(
      `The username ${new_user} is not available. Please enter a new username.`
    );
  } else {
